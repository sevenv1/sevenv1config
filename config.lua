...
runcode(function()
    local PrivateTexturePack = {Enabled = false}
    local TextureStyle = {Value = "Normal"}
    local objs
    local weldconnection
    PrivateTexturePack = GuiLibrary["ObjectsThatCanBeSaved"]["RenderWindow"]["Api"].CreateOptionsButton({
        ["Name"] = "PrivateTexturePack",
	["HoverText"] = "Meteor Texture Pack",
	["Function"] = function(callback)
            if callback then
                task.spawn(function()
    ...
    TextureStyle = PrivateTexturePack.CreateDropdown({
        Name = "Diamond Sword",
        List = {"Default", "Dynamic", "Custom"},
        Default = "Rach",
        Function = function()
            if PrivateTexturePack.Enabled then
                task.spawn(function()
                    PrivateTexturePack.ToggleButton(false)
                    task.wait()
                    PrivateTexturePack.ToggleButton(true)
                end)
            end
        end
    })
